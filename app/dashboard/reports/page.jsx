...
import { generatePDFHTML } from "@/lib/pdf-export"
  ...
  const handleExportReport = async () => {
    ...
    try {
      ...
      if (currentUser) {
        addAuditLog(
          "REPORT_GENERATED",
          `Report "${reportLabels[activeReport] || activeReport}" generated and exported by ${currentUser.name} - Format: PDF, Date: ${timestamp}`,
          currentUser.id
        )
      }
      ...
      switch (activeReport) {
        case "summary": {
          ...
          const pending = claims.filter(
            (c) => c.status === "submitted" || c.status === "pending-approval"
          )
              ...
              {canExport && (
                ...
                >
                  ...
                  <span className="whitespace-nowrap">
                    {exporting ? "Exporting..." : `Export ${reportLabels[activeReport] || "Report"}`}
                  </span>
                </button>
              )}
            </div>
