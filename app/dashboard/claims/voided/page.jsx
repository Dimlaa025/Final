...
export default function VoidedClaimsPage() {
  ...
  useEffect(() => {
    ...
    if (canViewAllClaims(currentUser)) {
      const voidedClaims = getClaims().filter(c => c.status === 'voided')
      setClaims(voidedClaims)
  ...
  return (
    <div className="p-6">
      <h1 className="text-3xl font-bold mb-6">Voided Claims ({claims.length})</h1>
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <table className="w-full">
          {/* Add table here similar to claims/page.jsx */}
