...
function MembersContent() {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e) => {
      if (e.key === 'we_care_members' || e.key === 'members') {
        loadMembers()
      }
    }
  ...
  // Helper for internal logic (MCB calculation)
  ...
  const handleFormSubmit = (memberData) => {
    if (editingMember) {
      const updatedMembers = members.map(m => m.id === editingMember.id ? { ...m, ...memberData } : m)
  ...
  return (
    <div className="p-4 sm:p-8">
      ...
      <SearchFilterBar onSearch={setSearchTerm} onFilterChange={setFilters} showFilters={true} />
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredMembers.map((member) => (
              <tr key={member.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm">₱{getMemberMCBUsed(member.id).toLocaleString()}</td>
