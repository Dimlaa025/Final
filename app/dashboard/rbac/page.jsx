...
import { generateSecurePassword, validatePassword } from "@/lib/utils"
...
function RBACSettingsContent() {
  ...
  const handlePasswordReset = (userId) => {
    ...
    if (success) {
      ...
      addAuditLog("PASSWORD_RESET", `Reset password for user ${targetUser.name}`, user.id)
  ...
  const handleApproveUser = (userId) => {
    ...
    if (userToApprove) {
      ...
      // Create a member record for the approved user
      const memberData = {
        id: userToApprove.memberId || undefined, // Use provided memberId or let system generate one
  ...
  const handleBulkReject = () => {
    ...
    if (confirm(`Are you sure you want to reject ${selectedPendingUsers.length} users?`)) {
      const allUsers = getUsers()
      let updatedUsers = allUsers.filter(u => !selectedPendingUsers.includes(u.id))
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      {/* Tab Navigation - Consistent across all devices */}
      ...
      {(activeTab === "users" || activeTab === "pending") && (
        <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  ...
                  >
                    ...
                    {roles.map((role, index) => (
                      <option key={`role-filter-${index}`} value={role.name}>{role.name}</option>
                    ))}
                  </select>
                </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        <option value="">Select Role</option>
                            {roles.map((role, index) => (
                              <option key={`bulk-${role.id}-${index}`} value={role.id}>{role.name}</option>
                            ))}
                      </select>
                      <button
              ...
              <div className="space-y-2 max-h-96 overflow-y-auto">
                ...
                {filteredUsers.map((u, index) => (
                  <div key={u.id || `user-${index}`} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg gap-4">
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                      <span className={`px-2 sm:px-3 py-1 rounded-full text-xs font-medium break-words whitespace-nowrap ${getRoleClass(u.role || 'Unknown')}`}>
                        {u.role || 'Unknown'}
                      </span>
                      ...
                      {canManageUsers(user) && (
                        <div className="flex items-center gap-2">
                          ...
                          >
                            {roles.map((role, index) => (
                              <option key={`user-role-${role.id}-${index}`} value={role.id}>{role.name}</option>
                            ))}
                          </select>
                        </div>
                      )}
          ...
          {activeTab === "pending" && (
            <div>
              {/* Bulk Operations for Pending Users */}
              ...
              <div className="space-y-4 max-h-96 overflow-y-auto">
                ...
                {pendingUsers.map((u, index) => (
                  <div key={u.id || `pending-${index}`} className="flex items-center justify-between p-4 border border-orange-200 bg-orange-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                      ...
                      <div>
                        ...
                        <p className="text-xs text-orange-600 mt-1">Registered: {new Date(u.createdAt).toLocaleDateString()}</p>
                      </div>
                    </div>
      ...
      {activeTab === "roles" && canManageRoles(user) && (
        <div className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm">
          ...
          <div className="space-y-4 max-h-96 md:max-h-none overflow-y-auto md:overflow-visible">
            {roles.map((role, index) => (
              <div key={role.id || `role-${index}`} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 border border-gray-200 rounded-lg gap-4">
                <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  {role.isSystem && (
                    <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs rounded whitespace-nowrap">System Role</span>
                  )}
                </div>
              </div>
            ))}
      ...
      {showEditRole && (
        <div className="fixed inset-0 bg-black/10 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-2xl max-h-[80vh] overflow-y-auto">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Edit Role: {editingRole?.name}</h3>
                            ...
                            <input
                              ...
                              onChange={(e) => {
                                if (e.target.checked) {
                                  ...
                                  setEditRole({...editRole, permissions: editRole.permissions.filter(p => p !== perm.id)})
                                }
                              }}
                              className="rounded border-gray-300 text-green-600 focus:ring-green-500"
                            />
