...
import { getPendingClaims, getClaims, getMembers, getAuditLogs, STORAGE_KEYS, getNotifications } from "@/lib/storage"
import { canApproveClaims, canViewReports, canExportReports, canViewAnalytics, canViewAuditLogs, canManageMembers } from "@/lib/permissions"
...
export default function DirectorDashboard({ user }) {
  ...
  const handleSearch = () => {
    ...
    const found = members.find(m =>
      `${m.firstName} ${m.lastName}`.toLowerCase().includes(searchName.toLowerCase())
    )

    if (found) {
      setSelectedMember(found)

      const claimsOfMember = claims.filter(c => c.memberId === found.id)
      setMemberClaims(claimsOfMember)
    } else {
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border space-y-3">
        ...
        {selectedMember && (
          <div className="mt-3 border-t pt-3 text-sm">
            ...
            <p><strong>Balance:</strong> ₱{selectedMember.balance || 0}</p>

            <div className="mt-2">
              <p className="font-medium">Claims:</p>
              {memberClaims.length > 0 ? (
                <ul className="list-disc ml-5">
                  {memberClaims.map((c, i) => (
                    <li key={i}>
        ...
        {searchName && !selectedMember && (
          <p className="text-red-500 text-sm">No member found</p>
        )}
      </div>
