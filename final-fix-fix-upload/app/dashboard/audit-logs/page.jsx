...
function AuditLogsContent() {
  ...
  const loadData = () => {
    ...
    if (currentUser) {
      addAuditLog(
        "AUDIT_LOG_VIEWED",
        `Audit logs accessed by ${currentUser.name} - Total logs: ${allLogs.length}`,
        currentUser.id
      )
    }
  }
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                  ...
                  <div className="relative dropdown-container">
                    <button
                      ...
                      className="flex items-center justify-center sm:justify-start gap-2 px-3 sm:px-4 py-2 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 transition text-sm sm:text-base min-w-0"
                    >
                      <span className="text-xs sm:text-sm text-gray-700 truncate">Sort</span>
                      <ChevronDown className="w-3 h-3 sm:w-4 sm:h-4 text-gray-500 flex-shrink-0" />
                    </button>
            ...
            <div className="hidden md:block bg-white rounded-lg shadow overflow-x-auto">
              <table className="w-full min-w-[600px] sm:min-w-[800px]">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-2 sm:px-3 md:px-6 py-3 text-left text-xs sm:text-sm font-semibold text-gray-900">
                      Timestamp
                    </th>
                    <th className="px-2 sm:px-3 md:px-6 py-3 text-left text-xs sm:text-sm font-semibold text-gray-900">
                      Action
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredLogs.length === 0 ? (
                    <tr>
                      <td
                        colSpan="5"
                        className="px-2 sm:px-3 md:px-6 py-6 sm:py-8 text-center text-xs sm:text-sm text-gray-500"
                      >
                        No audit logs found.
                      </td>
                    </tr>
                    ...
                    filteredLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-2 sm:px-3 md:px-6 py-3 sm:py-4 text-xs sm:text-sm text-gray-900 font-medium break-words">
