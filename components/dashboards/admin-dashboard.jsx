...
import { getClaims, getMembers, getAuditLogs, STORAGE_KEYS, getNotifications, hasPermission } from "@/lib/storage"
...
export default function AdminDashboard({ user }) {
  ...
  useEffect(() => {
    ...
    // Listen for storage changes to refresh stats automatically
  ...
  if (showAnalytics) {
    return (
      <AnalyticsDashboard user={user} analyticsData={analyticsData} dateRange={dateRange} setDateRange={setDateRange} onBack={() => setShowAnalytics(false)} />
    )
  }
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
            <DashboardCard
              ...
              icon={Clock}
              color="yellow"
              permission={canViewAllClaims(user)}
            />
            <DashboardCard
            ...
            <DashboardCard
              ...
              icon={AlertTriangle}
              color="red"
              permission={canApproveClaims(user)}
            />
            <DashboardCard
              title="Voided Claims"
              value={stats.voidedClaims}
              icon={AlertTriangle}
              color="orange"
              permission={canApproveClaims(user)}
            />
