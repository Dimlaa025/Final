...
// --- CONSOLIDATED DEPENDENT MANAGEMENT ---
...
// 2. Dual-Export Fix (Solves the "Export Not Found" build errors)
export function getDependentsByMember(memberId) {
  return getDependentsByMemberId(memberId);
}

export function getDependentsByMemberId(memberId) {
  if (!memberId) return [];
  const dependents = getDependents();
  // Using String() and trim() ensures "22-688-2525" matches correctly
  return dependents.filter(d => 
    String(d.memberId).trim() === String(memberId).trim()
  );
}

// 3. Normalized Create Function
export function createDependent(dependentData) {
  const dependents = getDependents();
  const memberId = String(dependentData.memberId).trim();

  // Check limit (Max 4)
  if (getDependentsByMemberId(memberId).length >= 4) {
  ...
  if (currentUser) {
    ...
    if (member) {
      addAuditLog("DEPENDENT_CREATED", `Added ${newDependent.name} for ${member.name}`, currentUser.id);
    }
  }
  return newDependent;
}

// 4. Standard UI Wrappers (Ensure only ONE addDependent exists)
...
export function updateDependent(id, updates) {
  ...
  if (index !== -1) {
    if (updates.memberId) updates.memberId = String(updates.memberId).trim();
...
export function deleteDependent(id) {
  ...
  localStorage.setItem(STORAGE_KEYS.DEPENDENTS, JSON.stringify(updated));
  const currentUser = getCurrentUser();
  if (currentUser) {
    addAuditLog("DEPENDENT_DELETED", `Removed dependent ${dep.name}`, currentUser.id);
  }
}
// Claims management
// Claims management
