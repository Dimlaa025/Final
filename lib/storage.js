...
export function getClaimsByDependentRelationship(user) {
  ...
  filteredClaims.forEach(claim => {
    if (claim.dependentId) {
      const dependent = dependents.find(d => d.id === claim.dependentId)
      if (dependent) {
...
export function getMemberVsDependentClaims(user) {
  ...
  const memberClaims = filteredClaims.filter(c => !c.dependentId)
  const dependentClaims = filteredClaims.filter(c => c.dependentId)

  return {
    member: {
      count: memberClaims.length,
      totalAmount: memberClaims.reduce((sum, c) => sum + parseFloat(c.amount), 0)
    },
    dependent: {
      count: dependentClaims.length,
      totalAmount: dependentClaims.reduce((sum, c) => sum + parseFloat(c.amount), 0)
    }
  }
}
